...
class _StatsScreenState extends State<StatsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer2<StatsProvider, TodoProvider>(
        builder: (context, stats, todoProvider, child) {
          if (stats.isLoading)
            return const Center(child: CircularProgressIndicator());

          final now = DateTime.now();
          final todayEntries = stats.entriesOnDate(now);
          final todayWorkMinutes = _totalMinutes(todayEntries);

          final weekEntries = stats.entriesInRange(_weekStart, _weekEnd);
          final monthEntries = stats.entriesInRange(_monthStart, _monthEnd);
          final weekWorkMinutes = _totalMinutes(weekEntries);
          final monthWorkMinutes = _totalMinutes(monthEntries);

          // === COMPLETED TODO TIME ===
          final completedTodos = todoProvider.completedTodos;
          final todoMinutesToday = completedTodos
              .where((t) => isSameDay(t.dueDate, now))
              .fold(0, (sum, t) => sum + (t.timeTakenMinutes ?? 0));
          final todoMinutesWeek = completedTodos
              .where(
                (t) =>
                    t.dueDate.isAfter(
                      _weekStart.subtract(const Duration(days: 1)),
                    ) &&
                    t.dueDate.isBefore(_weekEnd.add(const Duration(days: 1))),
              )
              .fold(0, (sum, t) => sum + (t.timeTakenMinutes ?? 0));

          final totalToday = todayWorkMinutes + todoMinutesToday;
          final totalWeek = weekWorkMinutes + todoMinutesWeek;
            ...
            child: Column(
              ...
              children: [
                ...
                _buildTimeSummary(
                  ...
                  totalWeek: totalWeek,
                  monthMinutes: monthWorkMinutes,
                  workToday: todayWorkMinutes,
                  todoToday: todoMinutesToday,
                  workWeek: weekWorkMinutes,
                  todoWeek: todoMinutesWeek,
                  stats: stats,
                  todayEntries: todayEntries,
                  weekEntries: weekEntries,
                  monthEntries: monthEntries,
  ...
  // === HELPER METHODS (copy-paste from your original) ===
  String _formatHM(int m) =>
      "${m ~/ 60}h ${m % 60}m".replaceAll(" 0m", "").replaceAll("0h ", "");
  int _totalMinutes(List<WorkEntry> entries) =>
      entries.fold(0, (s, e) => s + e.hours * 60 + e.minutes);
